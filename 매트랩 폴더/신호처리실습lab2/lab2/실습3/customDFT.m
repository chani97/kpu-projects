...
nk = 0:1:(x_len-1);   %dft 수식에서 k와 n이 될 변수 - 수식에서 0부터 시작.
%0부터 시작 필요.
Xk = zeros(1,x_len);
f_hat = linspace(-0.5,0.5-(1/x_len),x_len); %데모의 f_hat은 -0.5~0.5 범위이다.
N_mult = 0;
...
for k=1 : x_len
    ...
    if(x_len/2 + k <= x_len)        
        Xk(x_len/2 + k)  = sum(dftx); %주파수 영역으로 Xk를 집어넣는 코드 (양수 파트)
    else
